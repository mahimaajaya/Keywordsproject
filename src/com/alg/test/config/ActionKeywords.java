...
public class ActionKeywords extends BasePage{
	...
	public WebDriver launchBrowser(ExcelUtils xUtils, String sSheetName, int iTestCaseRowNum, int iTestStepRowNum){
		...
		try{
			if(System.getenv("ENV_REMOTE_LOCAL").contentEquals("Remote")){
	...
	public String commentCount(WebDriver wDriver, ExcelUtils xUtils, String sSheetName, int iTestCaseRowNum, int iTestStepRowNum){
		String sResult = "" ;
		//String sData = "";
		
		try{
							Log.info("Counting comment " );
				baseCommentCount(wDriver,getLocator(iTestStepRowNum, xUtils, sSheetName), 5);
				//Log.info("no of comment " +sResult );
			
		}catch(Exception e){
			sResult = "Error";
			Log.error("Error counting comment | Exception Description - " + e.getMessage());
		}
