...
public abstract class BasePage {
	...
	public String baseListDropDown(WebDriver wDriver, String sData, By locator, int iWaitInSeconds) throws Exception{
		...
		List<WebElement> list = wDriver.findElements(locator);
...
public String baseCommentCount(WebDriver wDriver, By locator, int iWaitInSeconds) throws Exception{
		...
		for(int i=0; i<list.size(); i++){
			String Str_count = list.get(i).getText();
			int count = Integer.parseInt(Str_count);
			total = total +count;
			}
		
		sResult = String.valueOf(total);		
		return sResult;
	}
	...
	public WebDriver startChrome() throws Exception{
		...
		System.setProperty("webdriver.chrome.driver",
				"C:\\Mahimaa\\Working\\KeywordFramework\\KeywordWorkspace\\KeywordsProject\\chromedriver.exe");
