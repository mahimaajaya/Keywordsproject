...
public class Executor extends BaseTest{
	...
	public static String sResultFile = "";
	...
	public void beforeClass(){
		...
		sResultFolder = Utils.createDirectory(BaseTest.sMasterDirPath, pParamPropFile.getProperty("Component"));
		Log.info("Directory created");
		
		//Copy Result file to above directory
		try{
			Log.info("Copying the data engine");
			Utils.copyFileToDirectory(System.getenv("ENV_DATA_ENGINE"), sResultFolder);
		}catch(Exception e){
			Log.error("Error copying the data engine | Exception Description - " + e.getMessage());
		}
		
		//Set the excel file object
		try{
			Log.info("Setting the data engine object");
			sResultFile = sResultFolder + System.getenv("ENV_DATA_ENGINE_FILE");
